...
export function dashboardRouter(pool) {
        ...
        `,
          ...
          `,
            ...
            const homesWithRooms = homes.map((home) => {
                ...
                // include unassigned devices that belong to the home but not to a room
                const unassignedDevices = devices.filter(
                    (d) => d.home_id === home.home_id && (d.room_id === null || d.room_id === undefined)
                );
    ...
    router.get("/dashboard/device-live", async (req, res) => {
        ...
        `,
            ...
            const row = rows[0];
            const values =
                typeof row.values_json === "string" ? JSON.parse(row.values_json) : row.values_json || {};
            const confidence =
                typeof row.confidence_json === "string"
                    ? JSON.parse(row.confidence_json)
                    : row.confidence_json || null;
            const flags =
                typeof row.flags_json === "string" ? JSON.parse(row.flags_json) : row.flags_json || null;
